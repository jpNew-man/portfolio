...
$(document).ready(function(){
    ...
    setTimeout(function(){
        $('#tree1').fadeOut(700);
        $('#section1 nav').fadeIn(700);
    },1400);
    
    $('.gnb li p').not('#section1 .gnb li p').hide(function(){
        $('nav').not('#section1 nav').mouseover(function(){
            $('.gnb p').not('#section1 .gnb p').stop().fadeIn();            
        });
         $('nav').not('#section1 nav').mouseout(function(){
            $('.gnb p').not('#section1 .gnb p').stop().fadeOut();
        });
    });
});
    ...
    $('.slider li img').mouseover(function(){
        $(this).stop().animate({marginTop:"-100px"});
        $(this).siblings("p").show(500);
    });

    $('.slider li img').mouseout(function(){
        $(this).stop().animate({marginTop:"0px"});
        $(this).siblings("p").hide();
    });

    var soapSlider = setInterval(function(){
        $(".slider").animate({
                marginLeft:"-=330px"
            },2000, function(){
                $(".slider li:first").appendTo(".slider");
                
                $(".slider").stop().css("margin-left", "0");
            });
